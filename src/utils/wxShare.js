...
const wxShare = {
	...
	_wxConfigJSSDK(shareSign) {
        ...
        // wx.config信息验证后会执行ready方法，所有接口调用都必须在config接口获得结果之后，config是一个客户端的异步操作，所以如果需要在页面加载时就调用相关接口，则须把相关接口放在ready函数中调用来确保正确执行。对于用户触发时才调用的接口，则可以直接调用，不需要放在ready函数中。
		wx.ready(function() {
...
// 文档：vue引入微信jssdk（网页调微信分享）http://note.youdao.com/noteshare?id=419ad24af007c5559e40a107692b6e1e&sub=ec512ec8444a3171d1004ecd702a253c