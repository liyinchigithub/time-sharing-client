...
export default {
  name: 'messageDetails', // 消息详情页（房源推送）
  components: {},
  data() {
    return {
      housingPushID: '', // 房源推送ID
      messageType: '', // 消息类型
      messageSendDateTime: '', // 发送时间
      sourceOfUser: '', //来源用户
      favoriteResults: '', // 咨询结果
      crrelationPropertiesID: '', // 房源推送ID，关联房源ID
      LeasableTime: '', // 租赁时间段
  ...
  mounted() {
    ...
    // 接收路由地址传参
    this.housingPushID = this.$route.params.housingPushID
    console.log(`咨询单ID：${this.housingPushID}`)
  }
}
</script>
