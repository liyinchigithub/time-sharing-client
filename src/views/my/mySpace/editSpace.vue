...
<template>
  <div>
    <!-- 顶栏、编辑空间 -->
    <van-nav-bar title="编辑空间" left-arrow @click-left="onClickLeft" :fixed="true" :border="true" />
    <!-- 表单 -->
    <div class="form">
...
import { createSpace, getSpaceDetail } from '@/api/my/mySpace/mySpace.js' // 引入后端api
export default {
  ...
  data() {
    return {
      ...
      spaceImages: [], // 空间详情图片，存放图片上传服务器后存放服务器所在的地址（存放的是file.file.path）（第一张图将作为封面）
      ...
      createSpace(data, headers)
        .then(response => {
          ...
          if (response.msg === '操作成功' || response.code === 0) {
            // Notify 成功提示
            Notify({ type: 'primary', message: '保存成功', duration: 1500 })
            this.$router.push('/spaceList')
          } else {
            // Notify 失败提示
            Notify({ type: 'warning', message: '保存失败', duration: 1500 })
          }
        })
        .catch(error => {
          console.log(error)
      ...
      axios
        ...
        .then(response => {
          ...
          if (response.data.code !== 0 || response.data.msg !== '操作成功') {
            ...
            // 响应body返回图片存储服务器所在路径
            console.log('this.spaceLogo', response.data.data)
      ...
      axios
        ...
        .then(response => {
          ...
          if (response.data.code !== 0 || response.data.msg !== '操作成功') {
            ...
            Toast.success('图片上传成功')
            // 响应body返回图片存储服务器所在路径
            console.log('this.spaceImages', response.data.data)
            this.spaceImages.push(response.data.data)
          }
        })
        .catch(error => {
          console.log(error)
    ...
    getSpaceDetail(data, headers)
      .then(response => {
        ...
        if (response.msg !== '操作成功' || response.code !== 0) {
          ...
          console.log(JSON.stringify(response.data))
          // 存储数据
          this.spaceStatus = response.data.status // 空间状态
          this.spaceAffiliatedBusinessesID = response.data.bid // 空间所属商家空间ID
          this.spaceID = response.data.id // 空间详情页，空间ID
          this.spaceName = response.data.name // 空间名称
          this.spaceDetailedIntroduction = response.data.description // 空间介绍
          this.spaceMerchantDetailsImagesLogoPre.push({ url: response.data.logo }) // logo图片(预览)
          this.spaceLogo.push(response.data.logo)
          // 图片数组转格式，元素符合vant van-uploader fileList 预览格式
          var array = response.data.images.split(',')
          for (let index = 0; index < array.length; index++) {
            const element = array[index]
            this.spaceMerchantDetailsImagesPre.push({ url: element }) // 图片组(预览)
          }
          this.spaceImages = response.data.images.split(',') // 图片组(注意：这边后端返回图片是字符串用逗号隔开并且有反斜杠，需要去掉反斜杠并转成数组)
          this.spaceDistrict = this.spacePosition.split('/')[2]
          this.spaceProvinces = response.data.province // 省
          this.spaceCity = response.data.city // 市区
          this.spaceDistrict = response.data.district // 区
          this.spacePosition = ``
          this.spaceDetailedAddress = response.data.address // 空间地址
          // 判断商家空间状态
